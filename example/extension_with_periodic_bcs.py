...
if __name__ == "__main__":
    ...
    numbers_of_defects_per_dimension = [4,]

    phasefield_volume_fraction_stepsize = 0.02
    ...
    collision_distance = 6 / number_of_cells_along_edge
    collision_distance_hard = collision_distance * 0.50
    ...
    penalty_weight = 0.200
    # penalty_weight = 0.300
    ...
    boundary_bot, boundary_rhs, boundary_top, boundary_lhs = utility.boundary_subdomains(mesh)
    boundary_markers = dolfin.MeshFunction('size_t', mesh, mesh.geometric_dimension()-1)
    ...
    ds = dolfin.ds(subdomain_data=boundary_markers)


    ### Function spaces

    fe_u = VectorElement('CG', mesh.cell_name(), 1) # Displacements
    fe_p = FiniteElement('CG', mesh.cell_name(), 1) # Phasefield
    fe_m = FiniteElement( 'R', mesh.cell_name(), 0) # Lagrange multiplier
    fe_w = MixedElement([fe_u, fe_m, fe_m, fe_m, fe_m])

    if periodic_boundary_conditions:
        pbc = utility.PeriodicBoundaries(mesh)
        V_w = FunctionSpace(mesh, fe_w, constrained_domain=pbc)
        V_p = FunctionSpace(mesh, fe_p, constrained_domain=pbc)
    else:
        V_w = FunctionSpace(mesh, fe_w)
        V_p = FunctionSpace(mesh, fe_p)

    w = Function(V_w, name="mixed")
    p = Function(V_p, name="phasefield")

    V_u, V_m1, V_m2, V_m3, V_m4 = V_w.split()
    u, m1, m2, m3, m4 = split(w)
    ...
    ### Average normal boundary displacement constraints

    # NOTE: Because of periodic boundary conditions, it is sufficient
    #       to define the constraints on right and top boundaries only.

    uN_bar = Constant(boundary_displacement_value)

    N = FacetNormal(mesh)
    uN = dolfin.dot(u, N)

    C_bcs = m1 * (uN-uN_bar) * ds(subdomain_id_bot) \
          + m2 * (uN-uN_bar) * ds(subdomain_id_rhs) \
          + m3 * (uN-uN_bar) * ds(subdomain_id_top) \
          + m4 * (uN-uN_bar) * ds(subdomain_id_lhs)
    ...
    dolfin.solve(F==0, w, bcs)

    undamaged_solution_vector = w.vector().get_local()
    ...
    for defect_coordinates_i in list_of_defect_coordinates:
        ...
        if save_results:
            ...
            solution_writer = utility.SolutionWriter(
                results_outdir_functions, w.split()[0], p,
                writing_period=function_writing_period)
        ...
        optimizer = optim.TopologyOptimizer(F, W, P, C, p, p_locals,
            w, bcs, external_callable=solution_writer.periodic_write)

        # Reinitialize the undamged displacement solution
        w.vector()[:] = undamaged_solution_vector
