...
class SimpleTimer:
    ...
    def __exit__(self, *exc_info):
        ...
        print(f'{self.message}: {self.elapsed:.3g} (sec.)')
...
class PeriodicExpression(dolfin.UserExpression):
    ...
    def eval(self, value, x):
        ...
        if self.mirror_x and num_periods_x % 2:
            ...
            dx = self._x0 - x + (num_periods_x+1.0)*self._period_x
        else:
        ...
        if self.mirror_y and num_periods_y % 2:
            ...
            dy = self._y0 - y + (num_periods_y+1.0)*self._period_y
        else:
...
class SimpleSubDomain(dolfin.SubDomain):
    def __init__(self, inside:'inside(x:np.ndarray)->bool'):
        super().__init__()
        try:
            b = inside(np.zeros((3,)))
            if b is None: raise TypeError
        except:
            raise TypeError('Parameter `inside` must be a function whose signature '
                            'is equivalent to `inside(x:np.ndarray)->bool`')
        self._inside = inside
...
def solve_compliance_maximization_problem(
    ...
    p_mean_constraint = Constant(0.0)

    if constrained_subdomain_functions is None:
        Cs = [(p-p_mean_constraint)*dolfin.dx(domain=mesh),]
    else:

        if not isinstance(constrained_subdomain_functions, (list, tuple)):
            constrained_subdomain_functions = (constrained_subdomain_functions,)

        constrained_subdomains = [SimpleSubDomain(subdomain_function_i)
            for subdomain_function_i in constrained_subdomain_functions]

        dx_Cs, dx_C0 = subdomain_integration_measures(mesh, constrained_subdomains)
        # NOTE: `dx_C0` corresponds to the measure `dx(domain=mesh)-sum(dx_Cs)`

        Cs = [(p-p_mean_constraint)*dx_Ci for dx_Ci in dx_Cs]
        if dx_C0.subdomain_data().array().any(): Cs.append(p*dx_C0)
    ...
    try:
        ...
        while phasefield_fraction_i < maximum_phasefield_fraction:
            ...
            try:
                ...
                p_mean_constraint.assign(phasefield_fraction_i)
    ...
    return iterations_failed, energy_vs_iterations, \
           ...
           optimizer, p_locals, p_mean_constraint


def subdomain_integration_measures(mesh, subdomains, subdomain_dim=None):
    '''
    Returns
    -------
    (list of) dolfin.Measure('s):
        The integration measures corresponding to the `subdomains`.
    dolfin.Measure:
        The integration measure for the subdomain not covered by `subdomains`.
    ...
    if not isinstance(mesh, dolfin.Mesh):
        raise TypeError('Parameter `mesh` must be in instance of `dolfin.Mesh`')
    ...
    if not all(isinstance(s, dolfin.SubDomain) for s in subdomains):
        raise TypeError('Parameter `subdomains` must be a (`list` '
                        'or `tuple` of) `dolfin.SubDomain`(s)')

    if subdomain_dim is None:
        subdomain_dim = mesh.geometric_dimension()

    dx_subdomains = []

    for subdomain_i in subdomains:
        mf = dolfin.MeshFunction('size_t', mesh, subdomain_dim, value=0)
        subdomain_i.mark(mf, DUMMY_ID)
        dx_subdomains.append(dolfin.dx(DUMMY_ID, domain=mesh, subdomain_data=mf))

    mf = dolfin.MeshFunction('size_t', mesh, subdomain_dim, value=0)
    mf.array()[sum(dx.subdomain_data().array() for dx in dx_subdomains)==0] = DUMMY_ID
    dx_compliment = dolfin.dx(DUMMY_ID, domain=mesh, subdomain_data=mf)

    return (dx_subdomains if return_as_sequence else dx_subdomains[0]), dx_compliment
...
def uniform_extension_bcs(V, method="biaxial"):
    '''
    ...
    if method == "biaxial":
        ...
        raise ValueError('Parameter `method`')
